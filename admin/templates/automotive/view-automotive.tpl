...
<div class="container">
    ...
    {/if}
        ...
        {/if}
        <tr class="{$class}">
			<td><a href="{$tconfig.tpanel_url}/index.php?file=at-automotive&mode=edit&iProductId={$db_automotive[i].iProductId}" title="">{$db_automotive[i].iYear}&nbsp;{$db_automotive[i].make}&nbsp;{$db_automotive[i].model}</a></td>					           
            <!--<td>{$db_automotive[i].store}</td>-->
            <td>{$db_automotive[i].Name}</td>
            <td>{$db_automotive[i].vType}</td>                        
            <td>{$db_automotive[i].fPrice}</td>
			<td>{$db_automotive[i].eStatus}</td>
			<td>
				<a href="{$tconfig.tpanel_url}/index.php?file=at-automotive&mode=edit&iProductId={$db_automotive[i].iProductId}" title=""><img src="{$tconfig.tpanel_img}icons/icon_edit.png" title="Edit" /></a>
				<a href="javascript:void(0);" title="Active" onclick="MakeAction('{$db_automotive[i].iProductId}','Active');"><img src="{$tconfig.tpanel_img}icons/icon_approve.png" title="Active" /></a>
				<a href="javascript:void(0);" title="Inactive" onclick="MakeAction('{$db_automotive[i].iProductId}','Inactive');"><img src="{$tconfig.tpanel_img}icons/icon_unapprove.png" title="Inactive" /></a>
				<a href="javascript:void(0);" title="Delete" onclick="MakeAction('{$db_automotive[i].iProductId}','Delete');"><img src="{$tconfig.tpanel_img}icons/icon_delete.png" title="Delete" /></a>
			</td>
			
			<td><input name="iProductId[]" type="checkbox" id="iId" value="{$db_automotive[i].iProductId}"/></td>
		</tr>
        {/section}
        {else}
        <tr>

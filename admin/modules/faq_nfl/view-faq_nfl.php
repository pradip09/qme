...
if($alp != ''){
    $ssql.= " AND vQuestion LIKE '".stripslashes($alp)."%'";

}

$sql_res = "SELECT * FROM faq_nfl WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM faq_nfl where 1 $ssql order by iFAQId DESC $var_limit ";	
$db_faq= $obj->MySQLSelect($sql_cus);
...
$sql_alp = "select vQuestion from faq_nfl where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vQuestion'], 0,1));
}

...
if($alp != ''){
    $ssql.= " AND vCategory LIKE '".stripslashes($alp)."%'";

}

$sql_res = "SELECT * FROM faq_category_nfl WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM faq_category_nfl where 1 $ssql order by iFAQCategoryId DESC $var_limit";	
$db_faqcat= $obj->MySQLSelect($sql_cus);
...
$sql_alp = "select vCategory from faq_category_nfl where 1=1";

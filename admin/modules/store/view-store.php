...
if($alp != ''){
    $ssql.= " AND vStoreCategory LIKE '".stripslashes($alp)."%'";
}
//$innerjoin = " INNER JOIN members m ON (s.iMemberId = m.iMemberId) ";
$sql_res = "SELECT * FROM store_category WHERE 1 ".$ssql;
#echo $sql_res;
...
$sql_cus = "SELECT * FROM store_category WHERE 1 $ssql order by iStoreCategoryId $var_limit";	
...
$sql_alp = "select vStoreCategory from store_category where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vStoreCategory'], 0,1));
}

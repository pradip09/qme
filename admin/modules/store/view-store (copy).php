...
if($alp != ''){
    $ssql.= " AND vItemName LIKE '".stripslashes($alp)."%'";

}
$innerjoin = " INNER JOIN members m ON (s.iMemberId = m.iMemberId) ";
$sql_res = "SELECT s.*,m.vName as Name FROM store s ".$innerjoin." WHERE 1 ".$ssql;
#echo $sql_res;
...
$sql_cus = "SELECT s.*,m.vName as Name FROM store s ".$innerjoin." where 1 $ssql order by iItemId $var_limit";	

$db_store= $obj->MySQLSelect($sql_cus);
...
$sql_alp = "select vItemName from store where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vItemName'], 0,1));
}

...
if($action == "add")
{
	...
	if($Data['iVideoAlbumId'] == 0)
	{
		...
		if($cnt > 0)
		{
			...
			header("Location: ".$tconfig["tpanel_url"]."/index.php?file=v-video&mode=view&var_msg=$var_msg");
			exit;
		}
		else
		{
...
if($action == "edit")
{
	...
	if($res)$var_msg = "Image is Updated Successfully."; else $var_msg="Eror-in Update.";
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=v-video&mode=view&var_msg=$var_msg");
	exit;
}
...
if($action == "Delete")
{
	...
	if($db_sql)$var_msg = "Record is Deleted Successfully.";else $var_msg="Eror-in Delete.";
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=v-video&mode=view&var_msg=$var_msg");
	exit;
}
...
if($action == "DeleteImage")
{
	...
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=v-video&mode=edit&iVideoId=$iVideoId&var_msg=$var_msg");
	exit;
}

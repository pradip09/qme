...
if($alp != ''){
    $ssql.= " AND vBlogCategory LIKE '".stripslashes($alp)."%'";

}

$sql_res = "SELECT * FROM blogcategory WHERE 1 $ssql ";	
...
$sql_cus = "SELECT * FROM blogcategory where 1 AND iMemberId=$memberId  $ssql order by iBlogCategoryId $var_limit";	
$db_viewblogcategory = $obj->MySQLSelect($sql_cus);
...
if(!count($db_viewblogcategory)>0 && $keyword != ""){
...
$sql_alp = "select vBlogCategory from blogcategory where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vBlogCategory'], 0,1));
}
...
$smarty->assign("db_viewblogcategory",$db_viewblogcategory);

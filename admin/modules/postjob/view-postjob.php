...
if($alp != ''){
    $ssql.= " AND p.vSkill LIKE '".stripslashes($alp)."%'";

}

$sql_res = "SELECT p.*, a.vFirstName, a.vLastName,p.vSkill,p.eStatus FROM post_job p INNER JOIN administrators a ON p.iAdminId = a.iAdminId WHERE 1=1 $ssql";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_postjob = "SELECT p.*, a.vFirstName, a.vLastName,p.vSkill,p.eStatus FROM post_job p INNER JOIN administrators a ON p.iAdminId = a.iAdminId WHERE 1=1 $ssql order by iPostJobId DESC $var_limit";
$db_postjob = $obj->MySQLSelect($sql_postjob);
//echo "<pre>";
//print_r($sql_postjob);exit;
...
$sql_alp = "SELECT p.*, a.vFirstName, a.vLastName FROM post_job p INNER JOIN administrators a ON p.iAdminId = a.iAdminId WHERE 1=1";

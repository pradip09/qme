...
if($alp != ''){
    $ssql.= " AND vPage LIKE '".stripslashes($alp)."%'";
}

$sql_res = "SELECT * FROM banner WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM banner where 1 $ssql order by iBannerId DESC $var_limit";	
$db_banner= $obj->MySQLSelect($sql_cus);
#ends here
...
$sql_alp = "select vPage from banner where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);
for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vPage'], 0,1));
}

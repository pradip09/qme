...
if($alp != ''){
    ...
    $ssql.= " AND vInterest LIKE '".stripslashes($alp)."%'";
}


$sql_res = "SELECT * FROM interest WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM interest where 1 $ssql order by iInterestId DESC $var_limit";
$db_interest= $obj->MySQLSelect($sql_cus);
#ends here
...
$sql_alp = "select vInterest from interest where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);
for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vInterest'], 0,1));
}

...
if($alp != ''){
    $ssql.= " AND vDisplayName LIKE '".stripslashes($alp)."%'";
}

$sql_res = "SELECT * FROM static_pages WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM static_pages where 1 $ssql order by iPageId DESC $var_limit";	
$db_static_pages= $obj->MySQLSelect($sql_cus);
...
if(!count($db_staticpage)>0 && $keyword != ""){
...
$sql_alp = "select vDisplayName from static_pages where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);
for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vDisplayName'], 0,1));
}

...
$sql_res = "SELECT * FROM agency_banner WHERE 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM agency_banner where 1 $ssql order by iAgencybannerId DESC $var_limit";	
$db_agn_banner= $obj->MySQLSelect($sql_cus);
#ends here
...
if(!count($db_agn_banner)>0 && $keyword != ""){
...
$sql_alp = "select vTitle from agency_banner where 1=1";

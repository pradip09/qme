...
if($alp != ''){
    $ssql.= " AND vStorePlanName LIKE '".stripslashes($alp)."%'";
}


$sql_res = "SELECT * FROM store_plan WHERE 1 ".$ssql;
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM store_plan WHERE 1 $ssql order by iStorePlanId $var_limit";	
$db_storeplan= $obj->MySQLSelect($sql_cus);
...
if(!count($db_storeplan)>0 && $keyword != ""){
...
$sql_alp = "select vStorePlanName from store_plan where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vStorePlanName'], 0,1));
}

...
if($option != '' && $keyword != ''){
    $ssql.= " AND ".stripslashes($option)." LIKE '".trim(stripslashes($keyword))."%'";
}
if($alp != ''){
    $ssql.= " AND vContestName LIKE '".stripslashes($alp)."%'";

}

$sql_res =  "SELECT * FROM contests  WHERE 1=1 $ssql";	
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
/*$sql_campaignjob = "SELECT * FROM post_campaign  WHERE 1=1 $ssql order by iContestId $var_limit";
$db_postcampaign = $obj->MySQLSelect($sql_campaignjob);*/


$sql_campaign = "SELECT * FROM contests  WHERE 1=1 $ssql order by iContestId DESC $var_limit";
...
$sql_alp = "SELECT * FROM contests ";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vContestName'], 0,1));
}

...
if($alp != ''){
    $ssql.= " AND vMake LIKE '".stripslashes($alp)."%'";

}

$sql_res = "SELECT * FROM make WHERE 1 $ssql ";
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

#Listing Starts from here
$sql_cus = "SELECT * FROM make where 1 $ssql order by imakeId DESC $var_limit";	
$db_make= $obj->MySQLSelect($sql_cus);
...
$sql_alp = "select vMake from make where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vMake'], 0,1));
}

...
if($action == "add")
{
	...
	$id = $obj->MySQLQueryPerform("faq_category",$Data,'insert');
	if($id)$var_msg = "FAQ Category is Added Successfully."; else $var_msg="Eror-in Add.";
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=fa-faqcat&mode=view&var_msg=$var_msg");
	exit;
}
...
if($action == "edit")
{
	...
	$res = $obj->MySQLQueryPerform("faq_category",$Data,'update',$where);
...
{	
	$iFAQCategoryId = $_POST['iFAQCategoryId'];
	$sql="Delete from faq_category where iFAQCategoryId='".$iFAQCategoryId."'";
...
{	
	...
	$where = " iFAQCategoryId IN (".$iFAQCategoryId.")";
	$sql="Delete from faq_category where ".$where; 
	$db_sql=$obj->sql_query($sql);	
	if($db_sql)
	{	

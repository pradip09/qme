...
if($alp != ''){
    $ssql.= " AND m.vModel LIKE '".stripslashes($alp)."%'";

}

$sql_cuss = "SELECT m.*,mk.vMake AS make FROM model AS m LEFT JOIN make AS mk ON(m.iMakeId = mk.iMakeId) where 1 $ssql ";	
$db_res = $obj->MySQLSelect($sql_cuss);
//echo $sql_cuss;exit;
$num_totrec = count($db_res);
include(TPATH_CLASS_GEN."paging.inc.php");

$sql_cus = "SELECT m.*,mk.vMake AS make FROM model AS m LEFT JOIN make AS mk ON(m.iMakeId = mk.iMakeId) where 1 $ssql order by iModelId DESC $var_limit";	
$db_model = $obj->MySQLSelect($sql_cus);
//echo $sql_cuss;exit;
...
$sql_alp = "select vModel from model where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vModel'], 0,1));
}

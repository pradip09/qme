...
if($alp != ''){
    $ssql.= " AND vSongName LIKE '".stripslashes($alp)."%'";

}
$innerjoin = " LEFT JOIN song_album sa ON (s.iSongAlbumId = sa.iSongAlbumId) LEFT JOIN members m ON (s.iMemberId = m.iMemberId) ";

$sql_res = "SELECT s.*,sa.vSongAlbum as SongAlbum,m.vName as Name FROM song s ".$innerjoin." WHERE 1 ".$ssql;
...
$sql_cus = "SELECT s.*,sa.vSongAlbum as SongAlbum,m.vName as Name FROM song s ".$innerjoin." where 1 $ssql order by iSongId $var_limit";	
$db_song= $obj->MySQLSelect($sql_cus);

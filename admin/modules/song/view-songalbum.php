...
if($alp != ''){
    $ssql.= " AND vSongAlbum LIKE '".stripslashes($alp)."%'";

}
$sql_res = "SELECT * FROM song_album WHERE 1 AND iMemberId=".$memberId." ".$ssql." ";	
...
$sql_cus = "SELECT * FROM song_album where 1 AND iMemberId=$memberId  $ssql order by iSongAlbumId $var_limit";	
...
if(!isset($start))
	...
	if($lastrec > $num_totrec)
		...
		if($num_totrec > 0 )
		{
			$recmsg11 = "Showing ".$startrec." - ".$lastrec." Records Of ".$num_totrec;
		}
		else
		{
			$recmsg11="No Records Found.";
		}
...
$sql_alp = "select vSongAlbum from song_album where 1=1";
$db_alp = $obj->MySQLSelect($sql_alp);

for($i=0;$i<count($db_alp);$i++){
    $db_alp[$i] = strtoupper(substr($db_alp[$i]['vSongAlbum'], 0,1));
}
...
$smarty->assign("recmsg11",$recmsg11);

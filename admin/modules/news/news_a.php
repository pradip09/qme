...
if($action == "add")
{
	...
	$id = $obj->MySQLQueryPerform("news",$Data,'insert');
	if(!is_dir($path."/".$id)){
		@mkdir($path."/".$id, 0777);
	...
	$where = " iNewsId = '".$id."'";
	$res = $obj->MySQLQueryPerform("news",$Data,'update',$where);
	
	if($res)$var_msg = "News is Added Successfully.";else $var_msg="Eror-in Add.";
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=n-news&mode=view&var_msg=$var_msg");
	}
}
...
{	
	$iNewsId = $_POST['iNewsId'];
	$sql="select vImage from news where iNewsId = ".$iNewsId; 
	$db_data=$obj->sql_query($sql);
	$eImg = $db_data[0]['vImage'];

	$sql="Delete from news where iNewsId='".$iNewsId."'"; 
	$db_sql=$obj->sql_query($sql);
	if($db_sql)
	{
		$var_msg = "News is Deleted Successfully.";
...
{	
	...
	foreach ($_POST['iNewsId'] as $id) {	
		$iNewsId = $iNewsId . $id .',';
		$sql="select vImage from news where iNewsId = ".$id; 
			$db_data=$obj->sql_query($sql);
			$eImg[] = $db_data[0]['vImage'];
	...
	$iNewsId = substr($iNewsId, 0, strlen($iNewsId)-1); 
    	...
    	$where = " iNewsId IN (".$iNewsId.")";
	$sql="Delete from news where ".$where; 
	$db_sql=$obj->sql_query($sql);	
	if($db_sql)
	{	
		$db_data = array();
...
{	
	...
	$where = " iNewsId = '".$iNewsId."'";
	$res = $obj->MySQLQueryPerform("news",$data_new,'update',$where);
	unlink(TPATH_SERVER_MUSIC_NEWS.'/'.$iNewsId."/2_".$_POST['vOldImage']);
	unlink(TPATH_SERVER_MUSIC_NEWS.'/'.$iNewsId."/1_".$_POST['vOldImage']);				
	unlink(TPATH_SERVER_MUSIC_NEWS.'/'.$iNewsId."/".$_POST['vOldImage']);
	...
	header("Location: ".$tconfig["tpanel_url"]."/index.php?file=n-news&mode=edit&iNewsId=$iNewsId&var_msg=$var_msg");
	exit;
}

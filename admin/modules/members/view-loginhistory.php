...
if($alp != ''){
    $ssql.= " AND concat(vFirstName,' ',vFirstName) LIKE '%".stripslashes($alp)."%'";
}

$sql = "SELECT * FROM login_histories WHERE lh.eType='User' $ssql";	
...
$sql_hist = "SELECT * FROM login_histories 
            WHERE eType='User' $ssql order by iLoginHistoryId DESC $var_limit";	
$db_hist = $obj->MySQLSelect($sql_hist);
#echo "<pre>";
#print_r($db_hist);exit;
...
$sql_alp = "select vFirstName from login_histories where 1=1 AND eType='User'";

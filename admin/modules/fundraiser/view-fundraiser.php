...
$sql_res = "SELECT f.*,m.vName FROM fundraiser_campaign f INNER JOIN members m ON (f.iMemberId = m.iMemberId) WHERE 1=1 $ssql";
$db_res = $obj->MySQLSelect($sql_res);
$num_totrec = count($db_res);


//$sql_res = "SELECT * FROM members";
//$db_mem = $obj->MySQLSelect($sql_res);
include(TPATH_CLASS_GEN."paging.inc.php");
//$innerjoin = " INNER JOIN members m ON (f.iMemberId = m.iMemberId)";

$sql_campaignjob = "SELECT f.*,m.vName FROM fundraiser_campaign f INNER JOIN members m ON (f.iMemberId = m.iMemberId) WHERE 1=1 ".$ssql." ORDER BY f.iCampaignId DESC $var_limit";
$db_fundcampaign = $obj->MySQLSelect($sql_campaignjob);
...
if(!count($db_fundcampaign)>0 && $keyword != ""){
...
$sql_alp = "SELECT f.* FROM fundraiser_campaign f INNER JOIN members m ON (f.iMemberId = m.iMemberId) WHERE 1=1";

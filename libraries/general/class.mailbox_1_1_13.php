	...
	*/
	public function MailDetail($iMailId,$viewtype)
	{
		...
		if($viewtype == 'inbox'){
			...
			{
				case "Member":
					$sql_info = "SELECT iMemberId,vName,vEmail FROM members WHERE iMemberId = '".$db_sql[0]['iToId']."'";
					break;
				case "Admin":
					$sql_info = "SELECT iAdminId,vName,vFirstName,vLastName,vEmail FROM administrators  WHERE iAdminId = '".$db_sql[0]['iToId']."'";
					break;	

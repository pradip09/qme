...
class zipcode_class {
   ...
   function get_zip_details($zip) {
      ...
      $sql = "SELECT flatitude AS lattitude, flatitude AS longitude, vCity, 
              FROM members 
              WHERE vZipCode='$zip'";
   ...
   function get_zip_point($zip) {
      ...
      $sql = "SELECT flatitude, flongitude from members WHERE vZipCode='$zip'";
      //echo $sql;exit; 
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
   ...
   function get_zips_in_range($zip, $range, $sort=1, $include_base) {
      ...
      $sql = "SELECT vZipCode, flatitude, flongitude FROM members ";
      if (!$include_base) $sql .= "WHERE vZipCode <> '$zip' AND ";
      else $sql .= "WHERE "; 
       $sql .= "flatitude BETWEEN '$min_lat' AND '$max_lat' 
               AND flongitude BETWEEN '$min_lon' AND '$max_lon'";
               //echo $sql;
      $r = mysql_query($sql);

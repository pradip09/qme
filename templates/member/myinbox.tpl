									...
									<td align="right">{$recmsg}&nbsp;&nbsp;<span class=bmatter>Pages</span> :{$page_link}</td>
										...
										<tr>
											<td  colspan="2" valign="top">
												...
												<div><table width="100%" border="0" class="table-border-status" cellspacing="0" cellpadding="0">
													{if  $msgs|@count gt 0}
													...
													{/if}
													<tr>
											<td width="4%" height="22" class="{$class}"><input  type="checkbox" name="ch[]" id='iId_{$msgs[j].iMailId}'  value="{$msgs[j].iMailId}" onchange="showdropdown();"/></td>
											{if $msgs[j].eRead eq 0}
											...
											{/if}
											<td width="30%" align="left" class="{$class}"><a href="{$site_url}/msgdetail/{$msgs[j].iMailId}/inbox" {if $msgs[j].eRead eq 0} {/if} class="sender-link" class="linktext">{$msgs[j].from_name}</a></td>

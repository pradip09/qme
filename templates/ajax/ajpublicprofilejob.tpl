...
<div class="jobbox" style="margin-left:50px;">
        	<div class="jobleft">
			{if $PostJobArr[i].iAdminId neq '0'}
				<img src="{$tconfig["tsite_upload_images_member"]}/{$mem_info[0].iMemberId}/2_{$mem_info[0].vProfileImage}" alt="" width="128" height="128"/><a href="#" class="postedby">Admin</a>
				<!--<img src="{$tconfig["front_images"]}admin_user.png" alt=" " title="" border="0" />-->
			{else}
				<img src="{$tconfig["tsite_upload_images_member"]}/{$mem_info[0].iMemberId}/2_{$mem_info[0].vProfileImage}" alt="" width="128" height="128" /><a href="#" class="postedby">{$mem_info[0].vName}</a>
		...
		<div class="likejob_box">
			<ul class="navjob">
				<li class="linkbtn"> <a href="javascript:void(0);"  onclick="qmelike('{$PostJobArr[i].iPostJobId}','{$mem_info[0].iMemberId}','Post_job','{$PostJobArr[i].iPostJobId}','1');" id="likejobs{$PostJobArr[i].iPostJobId}">QLike</a>&nbsp;|</li>
				<li class="linkbtn"> <a href="#Share" id="share{$PostJobArr[i].iPostJobId}" class="share">Share</a> &nbsp;|</li>
				<li class="linkbtn"><a href="#respond{$PostJobArr[i].iPostJobId}" id="respond1{$PostJobArr[i].iPostJobId}" class="respond">Respond</a> &nbsp;|</li>				
				{if $PostJobArr[i].iAdminId neq '0'}<li class="linkbtn">  </li>{else}<li class="linkbtn"> <a  href="javascript:void(0);" onclick="Checkrpt('{$mem_info[0].vName}','{$PostJobArr[i].iPostJobId}');">Report if this is not a job</a> </li>{/if}
			</ul>
		 </div>	
	...
	<div style="display:none">
        ...
        <div class="popuptext">{$PostJobArr[i].tDescription}</div></div>
        </div>
	</div>
...
<div style="text-align:center;color:#EEEDEB;">No Post Job available</div>
</div>
{/if}

{literal}
<script>
function Checkrpt(name,id){
	//alert('hello');
	var extra ='';
		extra+='&name='+name;
		extra+='&id='+id;

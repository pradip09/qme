...
{section name=i loop=$db_recentactivities}
	  ...
	  <div class="commentwrap">
		    <a href="{$site_url}/{$db_recentactivities[i].vMemberUrl}"><img src="{$db_recentactivities[i].vProfileImage}" alt=""  width="64" height="64"></a>
           		
		    	<div class="commentbox"><span class="bubble_arrow"></span>
	  		<div class="topcomment">
         			<span class="comment_title"><a style="color:#4D85AC" href="{$site_url}/{$db_recentactivities[i].vMemberUrl}">{$db_recentactivities[i].vName}</a></span> said <span class="comment_date">{$db_recentactivities[i].dAddedDate}</span><!--at <span class="comment_time" >05:39 Pm</span>--><span class="number_comments"><a id="countcom{$db_recentactivities[i].iRecentActivityId}"></a> <a href="javascript:void(0);" id="comment{$db_recentactivities[i].iRecentActivityId}" onclick="showcomment('{$db_recentactivities[i].iRecentActivityId}','{$mem_info[0].iMemberId}','recent_activity','{$db_recentactivities[i].iRecentActivityId}');"> Comments</a></span>
     
			      <p>
			       {$db_recentactivities[i].vDescription} {if $db_recentactivities[i].eType eq 'status_update'}:{$db_recentactivities[i].eNameActivity}{/if}
			       <a onclick="toggleText('{$db_recentactivities[i].iRecentActivityId}');" pid="{$db_recentactivities[i].iRecentActivityId}" style="cursor:pointer;">More</a>
			      </p>
			      <div class="slide{$db_recentactivities[i].iRecentActivityId}" style="display:none;">
				    <ul>
			...
			<div style="display:none;" class="dash_image">
			      <ul>
					{if $db_recentactivities[i].vImage neq ''}
					{if $db_recentactivities[i]['eType'] == 'approve_friend_request' || $db_recentactivities[i]['eType'] == 'status_update'}
	  ...
	  <div class="likebox_activity">						 
		    <ul class="navact">
			      <li><a style="color: #999;" class="likeicon" href="javascript:void(0);" onclick="qmelike('{$db_recentactivities[i].iRecentActivityId}','{$mem_info[0].iMemberId}','recent_activity','{$db_recentactivities[i].iRecentActivityId}','1');" id="likedata{$db_recentactivities[i].iRecentActivityId}"> Q Like</a></li>
			      <li ><a style="color: #999;" href="#Share" id="share{$db_recentactivities[i].iRecentActivityId}" class="shareicon"> Share this</a></li>
			      <li ><a style="color: #999;" href="javascript:void(0);" onclick="delcomment('{$db_recentactivities[i].iRecentActivityId}');"  class="commenticon" >Delete</a></li>
			     {if $db_recentactivities[i].eType eq 'PostJob' || $db_recentactivities[i].eType eq 'Post_job'}
			      <li ><a style="color: #999;" href="#respond{$db_recentactivities[i].iRecentActivityId}" id="respond1{$db_recentactivities[i].iRecentActivityId}" class="commenticon">Respond</a></li>
			      {/if}
			      {if $db_recentactivities[i].iType eq 'Private'}
			     <li ><a style="color: #999;" href="#popmsg{$db_recentactivities[i].iRecentActivityId}"   class="commenticon" >Post Comment</a></li>
			      {/if}		       
		     </ul>		 
...
<div style="display:none">
	...
	<form id="frm_msg{$db_recentactivities[i].iRecentActivityId}" enctype="multipart/form-data"  method="POST" name="frm_msg">
		...
		<div class="btncommnet">
			<input class="btnbg " type="button" value="Send message" onclick="postMsg('Private','{$db_recentactivities[i].iRecentActivityId}','{$db_recentactivities[i].iPostMemberId}');" style="float:right;margin-right: 0px;">
			<div class="cl"></div>
		</div>
	</form>
	</div>

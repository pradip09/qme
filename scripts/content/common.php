<?php
//UPDATE  `qme`.`members` SET  `vMemberUrl` =  'parthmurari' WHERE  `members`.`iMemberId` =78;
...
/*$sql = "SELECT * FROM members ";
 ...
 for($i=0;$i< count($member);$i++){
     $vName = $member[$i]['vName'];
     $vName= str_replace(' ', '', $vName);
    $iMemberId = $member[$i]['iMemberId'];
    $EarnData[$i] = array(
              'vMemberUrl'=>$vName,
	            
              );
    $where = " iMemberId = '".$iMemberId."'";
     $id_points = $obj->MySQLQueryPerform("members",$EarnData[$i],'update',$where);
   echo "hello";
...
for($i=0;$i< count($member);$i++){
  $EarnData[$i] = array(
          ...
          $id_points = $obj->MySQLQueryPerform("qme_earnpoints",$EarnData[$i],'insert');
...
if($_COOKIE['check_me'])
{
    ...
    if($mem_info[0]['iSkillId'] == '' AND $mem_info[0]['iInterestId'] == '')
    {
       $close_msg = 1;
    }else{
       $close_msg = 0;
   ...
   $mem_info[0]['vStateSortCode'] = $memState[0]['vStateCode'];
    ...
    $sql_earnpnts="select * from qme_earnpoints where iMemberId ='".$_SESSION['iUserId']."'";
    $earn_pnts = $obj->MySQLSelect($sql_earnpnts);
    $tot_earnpnt= $earn_pnts[0]['Total_earnpoints'];
    
    $sql_purchasepnts="select * from qme_purchagepnts where iMemberId ='".$_SESSION['iUserId']."'";
    $purchase_pnts = $obj->MySQLSelect($sql_purchasepnts);
    $tot_pursepnt = $purchase_pnts[0]['Total_purchagepoints'];    	
    $Tot_pnts= $tot_earnpnt + $tot_pursepnt ;
   
    //echo "<pre>";
    //print_r($tot_earnpnt);exit;$close_msg
    
    
    $smarty->assign("Tot_pnts",$Tot_pnts);
    $smarty->assign("close_msg",$close_msg);

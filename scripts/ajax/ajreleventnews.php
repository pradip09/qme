<?php
	...
	for($i=0;$i<count($relevent_news);$i++)
	{
		if($relevent_news[$i]['vImage'] =='' && !is_file(TPATH_SERVER_MUSIC_NEWS."/".$relevent_news[$i]['vImage']))
		{
		  ...
		  $relevent_news[$i][dAddedDate]=date("M d, Y",strtotime($relevent_news[$i][dAddedDate]));
		...
		$relevent_news[$i]['vTitle'] = $generalobj->limit_words($relevent_news[$i]['vTitle'],4);
		$relevent_news[$i]['vDescription'] = $generalobj->limit_words($relevent_news[$i]['vDescription'],12);
	}
	...
	for($i=0;$i<count($db_bloglistmatch);$i++)
	{
		...
		$db_bloglistmatch[$i]['vText'] = $generalobj->limit_words($db_bloglistmatch[$i]['vText'],12);
		$db_bloglistmatch[$i]['vBlogTitle'] = $generalobj->limit_words($db_bloglistmatch[$i]['vBlogTitle'],4);
		//echo "<pre>";

...
.submitbtn{float:left;  margin-left: 123px;}	
h1 {
margin-top: 0;
}
ul {
...
$contents.="<script type='text/javascript'>
	...
	var z = 0;
	//alert(document.openinviter.cont.length);
	for(z=1; z<=document.openinviter.cont.length;z++)
		{
			//	alert(1);
	
					document.getElementById('t_1').checked;
				//					alert(2);
	...
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			...
			if (count($contacts)==0)
				...
				{
				$contents.="<tr><td class='namefonttxt' width='15%'><input type='checkbox' onChange='toggleAll(this)' name='toggle_all' title='Select/Deselect all' checked>Invite?</td><td class='namefonttxt' width='40%'>Name</td>".($plugType == 'email' ?"<td class='namefonttxt' width='40%'>E-mail</td>":"")."</tr></table>";
				$contents.="<table width='100%' align='center' cellspacing='0' cellpadding='0'>";
				$counter=0;
				foreach ($contacts as $email=>$name)
					{
					$counter++;					
					$contents.="<tr><td class='namefonttxtsmall' width='15%' align='center'><input name='cont' id='t_{$counter}' value='{$counter}' type='checkbox'><input type='hidden' name='email_{$counter}' value='{$email}'><input type='hidden' name='name_{$counter}' value='{$name}'></td><td class='namefonttxtsmall' width='40%'>{$name}</td>".($plugType == 'email' ?"<td class='namefonttxtsmall' width='40%'>{$email}</td>":"")."</tr>";
					}
				$contents.="<tr><td colspan='".($plugType=='email'? "3":"2")."' style='padding:3px;' class='namefonttxtsmall'><div class='buttonfooter'><input type='submit' name='send' value='Send invites'></div></td></tr>";
				}
			$contents.="</table>";
			}
